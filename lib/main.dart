...
class _EventsViewState extends State<EventsView> {
  final db = FirebaseFirestore.instance;
  final _nameCtrl = TextEditingController();
  String _type = 'single', _stage = 'off-stage', _participation = 'open', _catType = 'general';
  final _pts1 = TextEditingController(text: '5'), _pts2 = TextEditingController(text: '3'), _pts3 = TextEditingController(text: '1');
  final _limit = TextEditingController(text: '3');
  ...
  void _addEvent() {
    ...
    Map<String, dynamic> data = {
      ...
      'category': _catType,
      'pts': [
        int.tryParse(_pts1.text) ?? 5, 
        int.tryParse(_pts2.text) ?? 3, 
        int.tryParse(_pts3.text) ?? 1
      ], 
      'createdAt': DateTime.now().millisecondsSinceEpoch
    };

    // Add limits safely
    data['limit'] = int.tryParse(_limit.text) ?? 3;
        ...
        onPressed: () => showDialog(context: context, builder: (c) => AlertDialog(
          ...
          content: SingleChildScrollView(
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              ...
              const SizedBox(height: 10),
              Row(children: [Expanded(child: _dd("Join", _participation, ['open', 'boys', 'girls'], (v)=>setState(()=>_participation=v!))), const SizedBox(width: 10), Expanded(child: _dd("Cat", _catType, ['general', 'specific'], (v)=>setState(()=>_catType=v!)))]),
              const SizedBox(height: 10),
              const Text("Points (1-2-3)", style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold)),
              Row(children: [Expanded(child: TextField(controller: _pts1)), const SizedBox(width: 5), Expanded(child: TextField(controller: _pts2)), const SizedBox(width: 5), Expanded(child: TextField(controller: _pts3))]),
              const SizedBox(height: 10),
              TextField(controller: _limit, decoration: const InputDecoration(labelText: "Limit/Groups"))
            ]),
          ),
            ...
            itemBuilder: (ctx, i) {
              ...
              return Card(
                child: ListTile(
                  ...
                  subtitle: Text("${e['type'].toUpperCase()} • ${e['stage']} • Pts: ${pts.join('-')}"),
  ...
  Widget _dd(String l, String v, List<String> i, void Function(String?)? c) {
    return DropdownButtonFormField<String>(
      value: v, 
      decoration: InputDecoration(labelText: l, contentPadding: const EdgeInsets.symmetric(horizontal: 10)), 
      items: i.map((e)=>DropdownMenuItem(value: e, child: Text(e.toUpperCase(), style: const TextStyle(fontSize: 12)))).toList(), 
      onChanged: c
    );
  }
}
